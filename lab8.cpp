...
int main()  // Main function is the starting point of the program execution.
{
    ...
    const double fPackCostA = 9.95, fPackCostB = 14.95, fPackCostC = 29.95; // Initialize the cost for each package in dollars.
    double fAccessHours, fBillAmount; // Declare the variables for the hours of internet access and the bill amount.
    ...
    switch (cChoice)    // Switch statement allows to select one of the code blocks to be executed.
    {
        case 'A':   // Execute the following code block if the package choice is A.
            ...
            if (fAccessHours < 0.0 || fAccessHours > 744.0)
            {
            ...
            if (fAccessHours <= 10.0) // If the number of hours doesn't exceed the limit, the bill amount is the package's price.
                cout << sBillAmountMessage << fPackCostA << endl;
            else    // If the number of hours exceeds the limit, calculate the bill amount.
            {
                fBillAmount = fPackCostA + (fAccessHours - 10.0) * 2.0;
        ...
        case 'B':   // Execute the following code block if the package choice is B.
            ...
            if (fAccessHours <= 20.0) // If the number of hours doesn't exceed the limit, the bill amount is the package's price.
                cout << sBillAmountMessage << fPackCostB << endl;
            else    // If the number of hours exceeds the limit, calculate the bill amount.
            {
                fBillAmount = fPackCostB + (fAccessHours - 20.0) * 1.0;
                cout << sBillAmountMessage << fBillAmount << endl;
            }
        ...
        case 'C':   // Execute the following code block if the package choice is C.
            ...
            cout << sBillAmountMessage << fPackCostC << endl;   // The bill amount for the unlimited access package is the price of the package.
